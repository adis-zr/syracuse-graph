...
\setlength{\parskip}{0.55em plus 0.1em minus 0.05em} 
\setlength{\parindent}{0pt} 
This paper builds an adaptive finite directed graph $G_D$ from the $2$-adic valuation
data of the $3n+1$ map.

The main result is an explicit bijection for the finite-distance layers of
this adaptive graph. The paper proves that the depth-$d$, distance-$k$ layer of $G_D$
is in canonical bijection with
\[
\Sfam_{d,k}
=
\{\Sigma\subseteq[d]:d\in\Sigma,\ |\Sigma|=2k+2\}.
\]
The paper also shows that $G_D$ encodes in its structure the logarithmic drift, 
thus turning the graph into a usable coordinate system rather than only a counting result.
\end{abstract}
\maketitle
...
\end{cases}
\]
Pick any positive integer and iterate.  The conjecture is that the sequence generated always
reaches~$1$.  We don't know why.  The standard references are Lagarias'
...
For the forward map, every integer has a unique successor, but a
chosen target can have many predecessors.  Thus one usually studies rooted
inverse trees or predecessor sets attached to specified nodes or cycles.  Their
branching and stopping-time statistics have been studied
extensively~\cite{ApplegateLagarias1995, ApplegateLagarias2003, Wirsching1998}.

The standard parity-dependent $3x+1$ map also has a $2$-adic form.  Bernstein
and Lagarias studied the continuous map
\[
T(x)=
\begin{cases}
(3x+1)/2, & x\text{ odd},\\
...
\end{cases}
\]
on $\mathbb Z_2$ and constructed a conjugacy with the $2$-adic shift, including
an analysis of the induced permutations modulo~$2^n$~\cite{Bernstein1994,
BernsteinLagarias1996}.  The accelerated odd-to-odd expression
\[
U(x)=\frac{3x+1}{2^{\nu_2(3x+1)}}
\]
is related but distinct: it is not a global continuous map on all of
$\mathbb Z_2$.  At the zero $x=-1/3$ of $3x+1$, the limiting odd part
depends on the $2$-adic direction of approach.  This paper does not rely on
such a global extension.  It studies the finite residue-class information
carried by the valuation $\nu_2(3x+1)$, and by the grouped valuation step
defined below.

In integer space, rooted inverse trees provide one way to organize
predecessors.  In $2$-adic residue space, the corresponding finite graphs need
not be trees.  A residue class modulo~$2^d$ can have multiple predecessors at
higher depth, and the dynamics between classes can cycle.  Laarhoven and
...
$G_D$ on vertices at varying depths.  It is neither a rooted integer predecessor tree nor
...
The results are deliberately finite and structural: they classify an adaptive
residue-depth graph, not integer orbits, and they do not prove convergence for
the $3n+1$ problem.
...
The graph is constructed from one arithmetic operation: the grouped odd-to-odd $3n+1$ step, and everything that follows depends on it. In this section we'll build the intuition first, then dive into definitions.
...
The graph is neither a rooted integer predecessor tree nor the de~Bruijn graph at a fixed modulus.  It is an adaptive structure: resolved vertices move to lower depth; unresolved vertices move to higher depth.  The interplay between these two directions is what gives $G_D$ its rigid internal structure.
...
at depth \(d\ge2\) reduces modulo \(2^{d-1}\) to an unresolved residue at depth \(d-1\).
Indeed, for \(1\le t\le d-2\),
\[
r_{d,t}\equiv 2^t(3^t)^{-1}-1 \pmod{2^d}
\]
reduces to the same defining congruence at depth \(d-1\).  For
\(t=d-1\) and \(t=d\), the reduction is instead
\[
r_{d,t}\equiv -1 \equiv 2^{d-1}-1 \pmod{2^{d-1}},
\]
the maximal unresolved residue at depth \(d-1\).  Thus each unresolved
vertex other than \((1,1)\) has an unresolved parent, and induction gives a
refinement path from \((1,1)\) to it.  Every direct return is a refinement child of an unresolved
vertex, so \((1,1)\) reaches all of \(C_D\).
...
Outside \(C_D\), the only unresolved vertices are boundary vertices.  The
terminal depth-\(D\) boundary vertices have no outgoing edges, while the
maximal boundary vertex at depth \(D-1\) has only refinement edges to terminal
boundary vertices.  A resolved vertex outside \(C_D\) has a valuation edge; if
that edge does not enter \(C_D\), then its target has depth
\(e=d-t-\rho<d\).  Thus any path outside \(C_D\) either terminates at the
boundary or follows valuation edges of strictly decreasing depth.  Neither
behavior permits a cycle.
...
The statements proved here therefore have a deliberately limited scope.  A vertex $(r,d)$ is a residue class, not an integer.  The integer $1$ is represented by an infinite compatible tower of residue classes, and integer dynamics require lift data beyond any fixed $G_D$.  No theorem in this paper proves, or is meant to imply, that all positive integer orbits reach $1$.  What the graph supplies is a finite, classifiable object: adaptive two-adic refinement produces a unique nontrivial SCC, pair-indexed valuation preimages, a top-bit symmetry, binomial distance layers, and a history-level log-drift functional.
