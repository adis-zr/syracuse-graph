...
The weight is not ornamental.  From the same equations,
\[
Y=\frac{3^tX+3^t-2^t}{2^{t+\rho}},
\]
and therefore, for a fixed finite valuation step,
\[
\log_2\frac{Y}{X}
=
t\log_2 3-(t+\rho)
...
Thus
\[
\log_2(Y/X)=t\log_2 3-(t+\rho)+O(1/X).
\]
The edge weight used below is the residue-class part of this Syracuse log-drift.  The weighted-history theorem says that the sum of these local log-drifts along any positive-distance valuation chain depends only on an explicit subset history $\Sigma\subseteq[d]$, not on the orientation of the residue lift.

The main point is still that the theorems below are finite graph theory.  A vertex is a residue class, not an integer orbit.  The results do not assert convergence of Collatz trajectories.  They identify a finite adaptive valuation graph whose recurrent core, preimage structure, quotient layers, and weighted histories can be classified exactly.

\subsection*{Main results}

The paper proves the following structural facts.

\begin{enumerate}
\item \textbf{Core theorem.}  For $D\ge3$, $G_D$ has a unique nontrivial strongly connected component $C_D$, explicitly described by exceptional residues and direct returns, with $|C_D|=D(D-2)$.
\item \textbf{Preimage-pair theorem.}  For a target of depth $e$, all valuation-edge preimages are indexed by the pairs $e<i<j\le D$.
\item \textbf{History bijection.}  Iterating those pairs gives a canonical coordinate on positive-distance vertices.  After the top-bit quotient, the depth-$d$, distance-$k$ layer is the binomial family $\Sfam_{d,k}$.
\item \textbf{Weight factorization.}  The Syracuse log-drift weight on a positive-distance chain factors through the same subset history as an alternating-gap functional.
\end{enumerate}

\section{Historical background and prior work}\label{sec:related}

The $3x+1$ problem has a large literature, and several strands are especially relevant to the present construction.  General surveys and bibliographies are indispensable for orientation: Lagarias' classical survey gives the standard formulation, early history, and many equivalent or generalized viewpoints \cite{Lagarias1985}; the later edited volume collects major expository and research articles on the problem \cite{Lagarias2010}; and the annotated bibliographies of Lagarias record the breadth of the literature through the first decades of systematic work \cite{LagariasAnnotated1999,LagariasAnnotated2009}.  Wirsching's monograph develops the $3n+1$ map as a dynamical system and treats predecessor sets, counting functions, and asymptotic questions in detail \cite{Wirsching1998}.  Conway's early article on unpredictable iterations is a reminder that Collatz-type maps are also natural objects in finite computation and undecidability-adjacent dynamics \cite{Conway1972}.  We use this literature mainly as context: the present results do not attempt to prove convergence of integer trajectories, but they do use a local valuation rule that comes from the odd-to-odd Syracuse dynamics.

A first line of prior work concerns stopping times, density theorems, and logarithmic drift.  Terras \cite{Terras1976} and Everett \cite{Everett1977} proved that almost all positive integers eventually fall below their starting value.  This was sharpened by Allouche \cite{Allouche1979} and Korec \cite{Korec1994}.  Krasikov and Lagarias developed difference-inequality methods for lower bounds on the number of integers whose iterates reach $1$ \cite{KrasikovLagarias2003}.  More recently, Tao proved that almost all Collatz orbits attain almost bounded values, in logarithmic density, using an approximate-transport argument for Syracuse random variables and estimates involving a skew random walk on $3$-adic cyclic groups \cite{Tao2022}.  In these analyses, the basic multiplicative bookkeeping is logarithmic: powers of $3$ contribute positive terms and powers of $2$ contribute negative terms.  Our edge weight $t\log_2 3-(t+\rho)$ is the finite adaptive-residue version of that same log-ratio functional.

A second line concerns cycles and valuation-gap accounting.  Garner's early work and Eliahou's lower bounds on nontrivial cycle lengths both use arithmetic constraints coming from the pattern of powers of two occurring along a hypothetical cycle \cite{Garner1981,Eliahou1993}.  Eliahou's paper is especially close in spirit to the weighted part of this paper: cycle obstructions are encoded through gap data and continued-fraction control of $\log_2 3$.  Modern refinements of cycle exclusions include Hercher's result ruling out Collatz $m$-cycles with $m\le91$ \cite{Hercher2023}.  The present paper is not a cycle-exclusion argument.  Its role is different: it shows that, inside the finite adaptive valuation graph, the same kind of log-drift bookkeeping factors through an explicit subset of depth coordinates.

A third line concerns inverse trees and predecessor sets.  The inverse $3x+1$ tree is already implicit in early work and was developed systematically in later counting and density results, including work of Applegate and Lagarias on the distribution of $3x+1$ trees and lower bounds for total stopping times \cite{ApplegateLagarias1995,ApplegateLagarias2003}.  Wirsching's book places predecessor sets at the center of the dynamical analysis \cite{Wirsching1998}.  Our preimage theorem is related in spirit but not in object: instead of enumerating integer predecessors of a fixed integer, it enumerates valuation-edge preimages of a residue vertex inside the finite adaptive graph.  The resulting index set is the elementary pair set $e<i<j\le D$, and iterating these pairs gives the subset histories in Theorem~\ref{thm:bijection}.

A fourth line is $2$-adic and symbolic.  For general background on $p$-adic numbers, see Gouvea \cite{Gouvea1997}.  Bernstein gave a non-iterative $2$-adic formulation of the conjecture \cite{Bernstein1994}.  Bernstein and Lagarias then studied the $3x+1$ conjugacy map $\Phi$ on the $2$-adic integers, conjugating the shift map to the $3x+1$ map and analyzing the induced permutations modulo $2^n$ \cite{BernsteinLagarias1996}.  Later work studied automorphisms and autoconjugacies of the $3x+1$ map, including the nontrivial autoconjugacy of Monks and Yazinski \cite{MonksYazinski2004}, and parity-vector viewpoints such as Rozier's treatment of parity sequences on $\mathbb Z_2$ \cite{Rozier2019}.  These works show that parity and $2$-adic coding are not auxiliary bookkeeping; they are central structure.  The present graph should be viewed in that same broad family, but with a different cutoff principle: it keeps vertices at varying depths and refines only when the valuation data have not yet determined a positive-depth transition.

A fifth line is explicitly graph-theoretic.  The ordinary Collatz graph is the directed graph on integers obtained by drawing arrows according to the Collatz rule; inverse versions produce rooted predecessor trees.  Laarhoven and de Weger studied congruence-class variants of the Collatz graph and proved that, for moduli that are powers of two, certain finite graphs are isomorphic to binary de Bruijn graphs; they also related the corresponding infinite graph isomorphism to the Bernstein--Lagarias conjugacy map \cite{LaarhovenDeWeger2013}.  This is the closest existing point of comparison.  Their finite graphs live at a fixed modulus $2^d$: every vertex has the same depth, and the de Bruijn structure comes from shifting binary information at that fixed depth.  The graph $G_D$ here is adaptive.  Its vertices range over all depths $1\le d\le D$, and an edge either refines a class to depth $d+1$ or follows a valuation-determined transition to a lower depth.  Thus the fixed-modulus de Bruijn regularity is replaced by a mixed-depth structure: a recurrent core, a truncation boundary, pair-indexed valuation preimages, top-bit quotient layers, and binomial histories.
...
The guiding principle is simple.  At depth $d$, the residue class determines
\[
t=\nuu(r+1)
\]
unless $r\equiv -1\pmod {2^d}$.  Once $t$ is known, the same depth-$d$ data determine $\rho=\nuu(3^tq-1)$ exactly unless the congruence persists all the way to the visible precision limit.  The exceptional residues are precisely the classes where the current depth has not yet pinned down a positive-depth valuation transition.  The graph therefore does the minimal thing: exceptional classes refine by one binary digit, and all other classes follow the determined valuation edge.
...
\subsection{A first example: \texorpdfstring{$G_4$}{G4}}

The graph $G_4$ is small enough to see all pieces at once.  The exceptional and direct-return residues are
\[
\begin{array}{c|c|c}
d & \Ecal_d & \Rcal_d\\ \hline
1 & \{1\} & \varnothing\\
2 & \{1,3\} & \varnothing\\
3 & \{3,5,7\} & \{1\}\\
4 & \{3,5,7,15\} & \{11,13\}.
...
The truncation boundary is
\[
\partial_4=\{(3,4),(5,4),(7,4),(15,4),(7,3)\}.
\]
The recurrent core is therefore
\[
C_4=\{(1,1),(1,2),(3,2),(1,3),(3,3),(5,3),(11,4),(13,4)\},
\]
so $|C_4|=8=4(4-2)$.

The remaining two vertices are at distance one from the core:
\[
(1,4)\to(1,2),\qquad (9,4)\to(3,2),
\]
and both valuation edges have $(t,\rho)=(1,1)$.  They form the two top-bit orientations of the same quotient history
\[
\Sigma=\{1,2,3,4\}\in\Sfam_{4,1}.
\]
The binomial formula gives
\[
2\binom{4-1}{2\cdot1+1}=2,
\]
exactly the two vertices above.  This example also shows why the cutoff boundary must be separated from the recurrent core: the terminal exceptional vertices at depth $4$, together with the maximal exceptional vertex $(7,3)$ just below them, do not return to the core inside $G_4$.
...
\]
Thus $K_D$ consists of the exceptional vertices and direct returns, except for the truncation boundary.  We call a strongly connected component \emph{trivial} if it consists of a single vertex.  The theorem below says that all nontrivial recurrence occurs in one component.
...
The next theorem is the basic enumerative mechanism.  It is target-first: fix a target and solve for all valuation-edge sources.  The important point is that a preimage is not indexed by an arbitrary residue.  It is indexed by the two new depth coordinates it inserts between the target depth and the source depth.

\begin{theorem}[Preimage-pair theorem]\label{thm:preimage}
Fix a target vertex $u=(s,e)\in V(G_D)$.  The valuation-edge preimages of $u$ are canonically indexed by pairs
\[
e<i<j\le D.
\]
...
\bibitem{Hercher2023}
C. Hercher.
There are no Collatz $m$-cycles with $m\le91$.
